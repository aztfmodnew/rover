...
register_rover_context() {
  ...
  ROVER_AGENT_DOCKER_IMAGE=$(curl -s https://hub.docker.com/v2/repositories/aztfmodnew/rover-agent/tags | jq -r ".results | sort_by(.tag_last_pushed) | reverse  | map(select(.name | contains(\"${docker_hub_suffix}\") ) | select(.name | contains(\"${ROVER_AGENT_TAG}\") ) ) | .[0].name")
